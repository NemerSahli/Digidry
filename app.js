...
app.post('/reset-pass', (req, res) => {
  ...
  User.findOne({ username: req.body.email }, function(err, dbres) {
    ...
    if (dbres.resetPassword) {
      ...
      return res.send({
        error: 250,
        data: 'you should confirm the email you got to reset your password'
      });
    }
  });
});
...
app.post('/reset', (req, res) => {
  ...
  User.findOne({ username: req.body.email }, function(err, dbres) {
    ...
    let mailBody = `<h3>You got this email to reset your password</h3>
            ...
            <a href="http://localhost:3000/reset/?activateLink=${
              dbres.acitivateKey
            }">Reset password</a>
            <p>yours sincerely</p>
            <p>DCI Fbw8 Team</p>`;
  ...
  if (req.query.activateLink) {
    ...
    User.findOne(query, function(err, doc) {
      ...
      doc.save(function(err, savedData) {
        ...
        res.send({
          error: 0,
          data: 'Thanks, now your password ready to reset'
        });
      });
    });
  }
